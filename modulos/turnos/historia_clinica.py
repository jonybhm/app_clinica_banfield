...
from PyQt5.QtGui import QPixmap, QTextCharFormat, QBrush, QColor, QIcon, QKeySequence
import os, datetime
...
class PantallaHistoriaClinica(QWidget):
    ...
    def buscar_turnos_ui(self):
        ...
        if estado == "PENDIENTE":
            ...
            for t in turnos:
                ...
                recepcionado = "✔️" in t.get("RECEPCION", "")
                tiene_hc = t.get("HCLIN") not in (None, 0)
                no_anulado = t.get("ANULADO", 0) == 0
                no_atendido = t.get("ATENDHC", 0) == 0

                #evolución real (no solo ATENDHC)
                tiene_evo = paciente_tiene_evolucion(codpac, fecha_turno)
            ...
            for t in turnos:
                atendhc = int(t.get("ATENDHC") or 0)
                atendido = int(t.get("ATENDIDO") or 0)
                codpac = t.get("CODPAC")

                tiene_evo = paciente_tiene_evolucion(codpac, fecha)

                if atendhc == 1 or atendido == 1 or tiene_evo:
                    turnos_filtrados.append(t)
        ...
        for fila in turnos:
            ...
            for col_idx, valor in enumerate(valores_visibles):
                ...
                if col_idx == 0:
                    ...
                    item.setData(Qt.UserRole + 1, fila)
                self.tabla.setItem(row_idx, col_idx, item)

            # Columna Atendido
            atendhc = fila.get("ATENDHC", 0)
            atendido = fila.get("ATENDIDO", 0)
            tiene_evo = paciente_tiene_evolucion(codpac, fecha)

            print(f"Paciente: {codpac} | ATENDHC: {atendhc} | ATENDIDO: {atendido} | EVO: {tiene_evo}")

            if atendhc == 1 or tiene_evo or atendido == 1:
    ...
    def abrir_dialogo_consulta(self):
        ...
        turno = item_paciente.data(Qt.UserRole + 1)
